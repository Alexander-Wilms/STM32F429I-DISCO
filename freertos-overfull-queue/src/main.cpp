...
void receive (void *)
{
	...
	while(1)
	{
		if( xQueueReceive( xQueue, rec, ( TickType_t ) portMAX_DELAY ) )
		{
			BSP_LCD_DisplayStringAtLine( 2, (uint8_t *) rec);
		}
	}
}
...
int main(void)
{
	...
	if( xQueue != 0 )
	{
		if( xQueueSend( xQueue, (void *) "ui", ( TickType_t ) portMAX_DELAY ) != pdPASS )
		{
	...
	xTaskCreate( (pdTASK_CODE)receive, 	   	"test",     configMINIMAL_STACK_SIZE, 0, TASK_PRIORITY, NULL);
