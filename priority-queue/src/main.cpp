...
void PQput(data * x)
{
	...
	{
		if( xSemaphoreTake( xSemaphoreMutex, ( TickType_t ) portMAX_DELAY ) == pdTRUE )
		{
			PQ.push_back(*x);
  ...
  while (true)
    {
	  stuff = 0;
      BSP_LED_Toggle(LED3);
      for( volatile long int counter = 0;counter<100000;counter++)
  		;
      BSP_LED_Toggle(LED3);
      for( volatile long int counter = 0;counter<200000;counter++)
        ;
      vTaskDelay(1);
    }
}
  ...
  while (true)
    {
	  stuff = 0;
      BSP_LED_Toggle(LED4);
      for( volatile long int counter = 0;counter<300000;counter++)
  		;
      BSP_LED_Toggle(LED4);
      for( volatile long int counter = 0;counter<400000;counter++)
        ;
      if(sent)
      {
			char h[2] = {0,0};
			...
			BSP_LCD_DisplayStringAtLine(1, (uint8_t *) h);
			BSP_LCD_DisplayStringAtLine(2, (uint8_t *) m);
			BSP_LCD_DisplayStringAtLine(3, (uint8_t *) l);
      }
      vTaskDelay(1);
    }
}
