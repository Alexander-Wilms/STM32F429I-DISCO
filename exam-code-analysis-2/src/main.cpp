...
{
	BSP_LCD_DisplayStringAtLine (j++, (uint8_t *) "1");
	for(int * i = data;i!=0;++i)
	{
		BSP_LCD_DisplayStringAtLine (j++, (uint8_t *) "2");
		xQueueReceive(Queue_1,i,portMAX_DELAY);
		BSP_LCD_DisplayStringAtLine (j++, (uint8_t *) "3");
		xQueueSend(Queue_2,i,portMAX_DELAY);
		//BSP_LCD_DisplayStringAtLine (j++, (uint8_t *) "Inhalt von i:");
		printf("Inhalt von i: %d", *i);
		i++;i--;
	}
	BSP_LCD_DisplayStringAtLine (j++, (uint8_t *) "1");
}
...
{
	BSP_LCD_DisplayStringAtLine (j++, (uint8_t *) "4");
	for(;;)
	{
		BSP_LCD_DisplayStringAtLine (j++, (uint8_t *) "5");
		vTaskSuspend(0);
		BSP_LCD_DisplayStringAtLine (j++, (uint8_t *) "6");
		xQueueSend(Queue_1,&message,portMAX_DELAY);
	}
}
uint32_t target[10]; // brackets may not be empty, otherwise: "error: storage size of 'target' isn't known"
static void low( void *x)
{
	BSP_LCD_DisplayStringAtLine (j++, (uint8_t *) "7");
	for(char * c=(char*)"210";*c != '0';++c)
	{
		BSP_LCD_DisplayStringAtLine (j++, (uint8_t *) "8");
		message = *c;
		BSP_LCD_DisplayStringAtLine (j++, (uint8_t *) "9");
		vTaskResume(h_medium);
		BSP_LCD_DisplayStringAtLine (j++, (uint8_t *) "10");
		xQueueReceive(Queue_2,0,portMAX_DELAY);
	}
	BSP_LCD_DisplayStringAtLine (j++, (uint8_t *) "11");
	//asm("break");
}
...
int main(void)
{
	...
	Queue_2 = xQueueCreate( 10, 0);

	xTaskCreate( (pdTASK_CODE)high, 	"high",    configMINIMAL_STACK_SIZE, 0, TASK_PRIORITY+2, NULL);
	xTaskCreate( (pdTASK_CODE)medium, 	"medium",  configMINIMAL_STACK_SIZE, 0, TASK_PRIORITY+1, &h_medium);
